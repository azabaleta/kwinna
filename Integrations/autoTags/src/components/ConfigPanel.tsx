...
import { type LayoutConfig, type GridLayout } from '@/utils/layout'
  ...
  return (
    <div className="flex flex-col h-full p-4 gap-6">
      ...
      <section className="space-y-2">
        <Label className="text-gray-200 text-sm font-semibold uppercase tracking-wide">
          Códigos
        </Label>
        <Textarea
          placeholder={'1234567\n7654321\n1111111'}
        ...
        <p className="text-xs text-gray-500">
          Un código por línea o separados por comas. 7 dígitos (checksum auto) u 8 dígitos.
        </p>
        {invalid.length > 0 && (
      ...
      <section className="space-y-4">
        ...
        <SliderField
          ...
          min={10}
          max={60}
          onChange={set('labelHeight')}
        />
        <SliderField
          label="Gap entre etiquetas"
          value={config.gap}
          min={0}
          max={10}
          onChange={set('gap')}
        ...
        {layout.totalPages > 1 && (
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-gray-400">
              Página {currentPage} / {layout.totalPages}
            </span>
            <Button
              variant="ghost"
              size="icon"
              onClick={() => onPageChange(Math.min(layout.totalPages, currentPage + 1))}
