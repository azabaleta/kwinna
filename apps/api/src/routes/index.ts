export { default as analyticsRoutes }    from "./analytics.routes";
export { default as authRoutes }         from "./auth.routes";
export { default as glossaryRoutes }     from "./glossary.routes";
export { default as creditNotesRoutes }  from "./credit-notes.routes";
export { default as posCustomersRoutes } from "./pos-customers.routes";
export { default as productRoutes }      from "./product.routes";
export { default as promoCodesRoutes }   from "./promo-codes.routes";
export { default as promoStripRoutes }   from "./promo-strip.routes";
export { default as shippingRoutes }     from "./shipping.routes";
export { default as reportsRoutes }      from "./reports.routes";
export { default as returnsRoutes }      from "./returns.routes";
export { default as saleRoutes }         from "./sale.routes";
export { default as stockRoutes }        from "./stock.routes";
export { default as stockBalancesRoutes} from "./stock-balances.routes";
export { default as usersRoutes }        from "./users.routes";
export { default as socialFormRoutes }   from "./social-form.routes";
export { default as planificacionRoutes } from "./planificacion.routes";
